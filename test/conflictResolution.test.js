import { AdvancedConflictResolver } from '../src/lib/conflictResolver';
describe('Advanced Conflict Resolution', () => {
  ...
  console.log("additional line");
            ...
            const result = await resolver.resolveConflict(baseContent, localContent, remoteContent, 'test.js', 'user1');
...
function test() {}`;
  ...
  "description": "A test package"
            ...
            const result = await resolver.resolveConflict(baseContent, localContent, remoteContent, 'package.json', 'user1');
            expect(result.strategy).toMatch(/merge/);
        });
    });
    describe('Conflict Metadata', () => {
        test('should provide detailed conflict metadata', async () => {
            ...
            const result = await resolver.resolveConflict(baseContent, localContent, remoteContent, 'test.txt', 'user1');
    ...
    describe('File Type Support', () => {
        test('should support JavaScript/TypeScript files', async () => {
            const result = await resolver.resolveConflict('const a = 1;', 'const a = 2;', 'const a = 3;', 'test.ts', 'user1');
            expect(result).toBeDefined();
            expect(result.strategy).toBeDefined();
        });
        test('should support Markdown files', async () => {
            const result = await resolver.resolveConflict('# Title\nContent', '# Title\nLocal content', '# Title\nRemote content', 'README.md', 'user1');
            expect(result).toBeDefined();
            expect(result.strategy).toBeDefined();
        });
        test('should handle unsupported file types gracefully', async () => {
            const result = await resolver.resolveConflict('binary content', 'local binary', 'remote binary', 'file.bin', 'user1');
            expect(result).toBeDefined();
            expect(result.strategy).toBeDefined();
        });
    });
    describe('Edge Cases', () => {
        test('should handle empty content', async () => {
            const result = await resolver.resolveConflict('', 'local content', 'remote content', 'test.txt', 'user1');
            expect(result).toBeDefined();
        ...
        test('should handle identical local and remote changes', async () => {
            ...
            const result = await resolver.resolveConflict(baseContent, sameContent, sameContent, 'test.txt', 'user1');
            expect(result.metadata?.autoResolved).toBe(true);
