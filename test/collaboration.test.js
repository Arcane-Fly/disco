...
import { initializeCollaborationManager } from '../src/lib/collaborationManager';
...
describe('Collaboration Manager', () => {
    ...
    test('should sync file to collaborators without error', async () => {
        await expect(collaborationManager.syncFileToCollaborators('test-container', '/test/file.txt', 'test content')).resolves.not.toThrow();
    });
});
