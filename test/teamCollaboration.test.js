...
describe('Team Collaboration Manager', () => {
    ...
    describe('Container Sharing', () => {
        test('should share container with team', () => {
            const team = manager.createTeam('owner', 'Test Team');
            const share = manager.shareContainer('container1', team.id, 'owner', 'write', undefined, { containerName: 'My Container', projectType: 'web' });
    ...
    describe('Workspace Management', () => {
        test('should create team workspace from template', () => {
            const team = manager.createTeam('owner', 'Test Team');
            const workspace = manager.createTeamWorkspace(team.id, 'owner', 'Frontend Project', 'React application workspace', 'frontend');
        ...
        test('should prevent non-authorized users from creating workspaces', () => {
            ...
            manager.addTeamMember(team.id, 'viewer1', 'owner', 'viewer');
            expect(() => {
                manager.createTeamWorkspace(team.id, 'viewer1', 'Unauthorized Workspace', 'Should fail', 'frontend');
    ...
    describe('Error Handling', () => {
        ...
        test('should handle invalid template names', () => {
            const team = manager.createTeam('owner', 'Test Team');
            expect(() => {
                manager.createTeamWorkspace(team.id, 'owner', 'Invalid Workspace', 'Should fail', 'nonexistent-template');
