...
describe('ChatGPT OAuth Integration', () => {
  ...
  describe('GET /oauth/authorize', () => {
    test('should validate required parameters', async () => {
      const response = await request(app).get('/oauth/authorize').expect(400);
  ...
  describe('MCP Manifest OAuth Configuration', () => {
    test('should have correct OAuth endpoints in manifest', () => {
      ...
      expect(manifest.authentication.authorization_url).toBe(
        'https://disco-mcp.up.railway.app/oauth/authorize'
      );
      expect(manifest.authentication.token_url).toBe(
        'https://disco-mcp.up.railway.app/oauth/token'
      );
      expect(manifest.authentication.redirect_uris).toContain(
        'https://chat.openai.com/oauth/callback'
      );
