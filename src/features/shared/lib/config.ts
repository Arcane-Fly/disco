...
import type { Environment, LogLevel, AppConfig } from '../types';
import { API_CONFIG, AUTH_CONFIG, UI_CONFIG, THEME_CONFIG } from './constants';
...
export const config: AppConfig = {
  ...
  ui: {
    theme: (process.env.NEXT_PUBLIC_DEFAULT_THEME as AppConfig['ui']['theme']) || THEME_CONFIG.DEFAULT_THEME,
...
export const API_ENDPOINTS = {
  ...
  CONTAINER_DETAILS: (id: string) => `/api/v1/containers/${id}`,
  CONTAINER_START: (id: string) => `/api/v1/containers/${id}/start`,
  CONTAINER_STOP: (id: string) => `/api/v1/containers/${id}/stop`,
  
  // Files
  FILES: '/api/v1/files',
  FILE_CONTENT: (path: string) => `/api/v1/files/content?path=${encodeURIComponent(path)}`,
  FILE_UPLOAD: '/api/v1/files/upload',
  FILE_DOWNLOAD: (path: string) => `/api/v1/files/download?path=${encodeURIComponent(path)}`,
  ...
  TERMINAL_SESSION: (id: string) => `/api/v1/terminal/sessions/${id}`,
