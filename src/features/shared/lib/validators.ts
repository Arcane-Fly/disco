...
export function validateFilePath(path: string): ValidationResult {
  return validateField('path', path, {
    ...
    custom: (p) => {
      ...
      if (p.includes('//')) return 'Path cannot contain "//"';
      if (!p.startsWith('/') && !p.match(/^[a-zA-Z]:/)) return 'Path must be absolute';
      return true;
    },
  });
}
...
export function validateMimeType(mimeType: string, allowedTypes: string[]): ValidationResult {
  return validateField('mimeType', mimeType, {
    required: true,
    custom: (type) => {
      if (typeof type !== 'string') return 'MIME type must be a string';
...
export function validateIPAddress(ip: string): ValidationResult {
  ...
  return validateField('ipAddress', ip, {
    required: true,
    custom: (address) => {
      if (typeof address !== 'string') return 'IP address must be a string';
...
export function validateContainerName(name: string): ValidationResult {
  return validateField('containerName', name, {
    ...
    custom: (n) => {
      if (typeof n !== 'string') return 'Container name must be a string';
...
export function validateEnvironmentVariable(name: string, value: string): ValidationResult {
  const nameResult = validateField('envName', name, {
    ...
    pattern: /^[A-Z][A-Z0-9_]*$/,
    custom: (n) => {
      if (typeof n !== 'string') return 'Environment variable name must be a string';
      if (n.includes('__')) return 'Environment variable name cannot contain consecutive underscores';
      return true;
    },
  });
  ...
  return validateField('envValue', value, {
    required: true,
    custom: (v) => {
      if (typeof v !== 'string') return 'Environment variable value must be a string';
