 ...
 */
router.post(
  '/:containerId/session/:sessionId/record/start',
...
router.post('/:containerId/execute', async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `💻 Executing command: "${command}" in container ${containerId}${terminalSession ? ` (session: ${terminalSession.id})` : ''}`
    );
  ...
  try {
    ...
    try {
      await executeCommandStreaming(
        ...
        cwd,
        env,
        (data: { type: string; data?: string; exitCode?: number; error?: string }) => {
