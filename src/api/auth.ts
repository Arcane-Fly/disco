...
router.get('/', (req: Request, res: Response) => {
  ...
  const responseData: AuthStatusResponse = {
    ...
    github_oauth: {
      ...
      callback_url:
        githubOAuthConfigured && !hasPlaceholders ? '/api/v1/auth/github/callback' : null,
  ...
  if (
    ...
    return res.status(503).json({
      ...
      error: {
        ...
        details: {
          issue:
            'Environment variables contain placeholder values instead of actual OAuth credentials',
...
router.get('/github/callback', async (req: Request, res: Response) => {
  try {
    ...
    if (isChatGPTFlow) {
      ...
      console.log(
        `🔐 GitHub user authenticated for ChatGPT OAuth: ${userData.login}, redirecting to consent`
      );
