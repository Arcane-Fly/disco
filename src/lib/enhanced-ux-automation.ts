import { EnhancedBrowserAutomationManager, VisualRegressionResult } from './enhanced-browser.js';
...
// Enhanced interfaces for strategic UX improvements
...
export interface LayoutChange {
  type: 'position' | 'size' | 'spacing' | 'alignment';
  element: string;
  delta: number;
  significance: number;
}

export interface UXAutomationAction {
  type: 'click' | 'type' | 'wait' | 'screenshot' | 'scroll' | 'validate' | 'analyze';
  selector?: string;
  text?: string;
  validation?: {
...
export interface UXAutomationResult {
  action: string;
  success: boolean;
  duration: number;
  accessibilityResults?: AccessibilityValidationResult;
  performanceMetrics?: PerformanceMetrics;
  semanticValidation?: SemanticAnalysisResult;
...
export interface EnhancedVisualRegressionResult extends VisualRegressionResult {
...
export class EnhancedUXAutomationManager extends EnhancedBrowserAutomationManager {
   ...
   */
  async performAdvancedVisualRegression(
    sessionId: string,
    pageId: string,
    testName: string,
    options: {
  ...
  ): Promise<EnhancedVisualRegressionResult> {
    const {
      threshold = 0.95,
  ...
  private async checkElementLabeling(element: any): Promise<boolean> {
  ...
  private async checkFocusIndicator(element: any): Promise<boolean> {
    const focusStyles = await element.evaluate((el: any) => {
      el.focus();
  ...
  */
  ...
  private identifyPerformanceRegressions(
    baseline: PerformanceMetrics,
    current: PerformanceMetrics
  ): string[] {
  ...
  private async calculatePageUsabilityScore(page: any): Promise<number> {
    let score = 100;
    ...
    try {
      ...
      if (!hasSearchFunctionality) score -= 5;
  ...
  private async performEnhancedTyping(page: any, action: UXAutomationAction): Promise<void> {
