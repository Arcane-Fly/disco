...
class TerminalSessionManager {
   ...
   */
  async stopRecording(sessionId: string): Promise<TerminalRecording | null> {
    const session = await this.getSession(sessionId);
   ...
   */
  async getRecording(recordingId: string): Promise<TerminalRecording | null> {
    try {
   ...
   */
  async getSessionRecordings(sessionId: string): Promise<TerminalRecording[]> {
    try {
      if (redisSessionManager.isConnected()) {
