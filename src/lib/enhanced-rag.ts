...
export class EnhancedRAGService {
  ...
  async indexCodebaseWithAST(
    container: any,
    options: { paths: string[]; excludePatterns: string[] }
  ) {
    ...
    console.log(
      `✅ Enhanced RAG indexing complete: ${indexedFiles}/${totalFiles} files, ${totalElements} code elements`
    );
  ...
  async performEnhancedSearch(
    ...
    uniqueResults.sort(
      (a, b) => b.score + (b.semanticSimilarity || 0) - (a.score + (a.semanticSimilarity || 0))
    );
...
router.get('/api/${this.extractEndpointName(query)}', async (req, res) => {
  ...
  private calculateTextSemanticSimilarity(line: string, query: string): number {
    ...
    for (const queryWord of queryWords) {
      if (
        lineWords.some(lineWord => lineWord.includes(queryWord) || queryWord.includes(lineWord))
      ) {
        matches++;
      }
    }
