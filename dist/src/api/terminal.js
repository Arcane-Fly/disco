 ...
 */
router.post('/:containerId/session', async (req, res) => {
    try {
        const { containerId } = req.params;
        const { sessionId, cwd, env } = req.body;
 ...
 */
router.get('/:containerId/sessions', async (req, res) => {
    try {
        const { containerId } = req.params;
        const userId = req.user.userId;
        // Verify container exists and user has access
 ...
 */
router.get('/:containerId/session/:sessionId/history', async (req, res) => {
    try {
        ...
        const history = await terminalSessionManager.getSessionHistory(sessionId, limit ? parseInt(limit) : 50, search);
        res.json({
            status: 'success',
            data: { history },
        });
 ...
 */
router.get('/:containerId/session/:sessionId/suggestions', async (req, res) => {
    try {
        const { containerId, sessionId } = req.params;
        ...
        const suggestions = await terminalSessionManager.getCommandSuggestions(sessionId, partial || '', limit ? parseInt(limit) : 10);
        res.json({
            status: 'success',
            data: { suggestions },
        });
 ...
 */
router.get('/:containerId/session/:sessionId/frequent', async (req, res) => {
    try {
        const { containerId, sessionId } = req.params;
        ...
        const frequentCommands = await terminalSessionManager.getFrequentCommands(sessionId, limit ? parseInt(limit) : 10);
        res.json({
            status: 'success',
 ...
 */
router.post('/:containerId/session/:sessionId/search', async (req, res) => {
    try {
        const { containerId, sessionId } = req.params;
 ...
 */
router.post('/:containerId/session/:sessionId/record/start', async (req, res) => {
    try {
        const { containerId, sessionId } = req.params;
        const userId = req.user.userId;
        // Verify session exists and user has access
 ...
 */
router.post('/:containerId/session/:sessionId/record/stop', async (req, res) => {
    try {
        const { containerId, sessionId } = req.params;
        const userId = req.user.userId;
        // Verify session exists and user has access
 ...
 */
router.get('/:containerId/session/:sessionId/recordings', async (req, res) => {
    try {
        const { containerId, sessionId } = req.params;
        const userId = req.user.userId;
        // Verify session exists and user has access
 ...
 */
router.get('/:containerId/recording/:recordingId', async (req, res) => {
    try {
 ...
 */
router.post('/:containerId/recording/:recordingId/replay', async (req, res) => {
    try {
        const { containerId, recordingId } = req.params;
 ...
 */
router.delete('/:containerId/session/:sessionId', async (req, res) => {
    try {
        const { containerId, sessionId } = req.params;
        const userId = req.user.userId;
        // Verify session exists and user has access
...
router.post('/:containerId/execute', async (req, res) => {
    try {
        const { containerId } = req.params;
        const { command, cwd, env, sessionId } = req.body;
        const userId = req.user.userId;
        ...
        if (sessionId) {
            ...
            if (!terminalSession ||
                terminalSession.containerId !== containerId ||
        ...
        if (terminalSession) {
            await terminalSessionManager.addCommandToHistory(terminalSession.id, command, result.output, result.exitCode, duration);
        }
        const response = {
            ...result,
            duration,
 ...
 */
router.post('/:containerId/stream', async (req, res) => {
    try {
        const { containerId } = req.params;
        ...
        const startTime = Date.now();
        try {
            await executeCommandStreaming(session.container, command, cwd, env, (data) => {
 ...
 */
router.get('/:containerId/history', async (req, res) => {
    try {
        const { containerId } = req.params;
        const userId = req.user.userId;
        const session = await containerManager.getSession(containerId);
 ...
 */
router.post('/:containerId/kill', async (req, res) => {
    try {
        const { containerId } = req.params;
...
async function executeCommand(container, command, cwd, env) {
    try {
...
async function executeCommandStreaming(container, command, cwd, env, onData) {
    // Read stream chunks and send them immediately
    async function readStream(reader, onData) {
        try {
...
async function getCommandHistory(container, sessionId) {
    ...
    try {
        ...
        if (historyResult.exitCode === 0) {
            ...
            const historyLines = historyResult.output
                .split('\n')
    ...
    return ['ls -la', 'pwd', 'git status', 'npm install', 'npm start'];
}
async function killProcess(container, pid, signal) {
    try {
