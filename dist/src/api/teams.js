 ...
 */
router.post('/:teamId/containers/:containerId/share', authMiddleware, async (req, res) => {
    try {
        ...
        const containerShare = teamCollaborationManager.shareContainer(containerId, teamId, sharedBy, accessLevel, expirationDate, metadata);
        res.json({
            success: true,
            containerShare: {
 ...
 */
router.post('/:teamId/workspaces', authMiddleware, async (req, res) => {
    try {
        const { teamId } = req.params;
        ...
        const workspace = teamCollaborationManager.createTeamWorkspace(teamId, createdBy, name, description, templateName, settings);
        res.status(201).json({
            success: true,
            workspace: {
                id: workspace.id,
 ...
 */
router.get('/:teamId/activity', authMiddleware, async (req, res) => {
    try {
        const { teamId } = req.params;
 ...
 */
router.get('/workspace-templates', authMiddleware, async (req, res) => {
    try {
 ...
 */
router.get('/containers/:containerId/access', authMiddleware, async (req, res) => {
    try {
        ...
        const hasAccess = teamCollaborationManager.canAccessContainer(containerId, userId, operation);
        res.json({
            containerId,
            operation,
            hasAccess,
