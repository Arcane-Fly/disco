...
import { ContractValidator, ErrorCode, ContractValidationError, createErrorEnvelope, } from '../lib/contractValidator.js';
const router = Router();
 ...
 */
router.post('/pinecone/upsert', async (req, res) => {
    try {
        const response = await validator.validateOperation('pinecone', 'upsert', req.body, async (request) => {
        ...
        res.status(500).json(createErrorEnvelope(ErrorCode.INTERNAL_ERROR, error instanceof Error ? error.message : 'Unknown error'));
    }
});
/**
 ...
 */
router.post('/pinecone/query', async (req, res) => {
    try {
        const response = await validator.validateOperation('pinecone', 'query', req.body, async (request) => {
...
router.post('/supabase/sql', async (req, res) => {
    try {
        const response = await validator.validateOperation('supabase', 'sql', req.body, async (request) => {
 ...
 */
router.post('/browserbase/navigate', async (req, res) => {
    try {
        const response = await validator.validateOperation('browserbase', 'navigate', req.body, async (request) => {
 ...
 */
router.post('/github/searchIssues', async (req, res) => {
    try {
        const response = await validator.validateOperation('github', 'searchIssues', req.body, async (request) => {
 ...
 */
router.get('/:service/:operation/:type', contractSchemaRateLimiter, (req, res) => {
    try {
        const { service, operation, type } = req.params;
        if (!['request', 'response'].includes(type)) {
            return res.status(400).json(createErrorEnvelope(ErrorCode.INVALID_INPUT, 'Type must be "request" or "response"'));
        }
        const fs = require('fs');
        const path = require('path');
        const contractsRoot = path.resolve(process.cwd(), 'contracts');
        const schemaPathUnresolved = path.join(contractsRoot, service, `${operation}.${type}.json`);
        let schemaPath;
        try {
            schemaPath = fs.realpathSync(schemaPathUnresolved);
        }
        catch (e) {
            return res.status(404).json(createErrorEnvelope(ErrorCode.NOT_FOUND, 'Schema not found'));
        }
        if (!schemaPath.startsWith(contractsRoot + path.sep)) {
            // Prevent access outside contracts directory
            return res.status(403).json(createErrorEnvelope(ErrorCode.INVALID_INPUT, 'Invalid schema path'));
        }
...
//# sourceMappingURL=contract-demo.js.map