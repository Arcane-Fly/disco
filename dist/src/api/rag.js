...
import { enhancedRAG } from '../lib/enhanced-rag.js';
const router = Router();
/**
 * POST /api/v1/rag/:containerId/search
 * Enhanced search for relevant code snippets using natural language with AST analysis
 */
router.post('/:containerId/search', async (req, res) => {
    try {
        const { containerId } = req.params;
        const { query, limit = 10, includeContext = true, useAST = true, semanticSearch = true, includeCodeSuggestions = true, } = req.body;
 ...
 */
router.post('/:containerId/index', async (req, res) => {
    try {
        const { containerId } = req.params;
 ...
 */
router.post('/:containerId/analyze', async (req, res) => {
    try {
        const { containerId } = req.params;
        const { filePath, analysisType = 'comprehensive', includeRefactoring = true, includeOptimization = true, includeDocumentation = true, } = req.body;
        const userId = req.user.userId;
...
async function performAICodeAnalysis(container, options) {
    try {
...
function generateRefactoringSuggestions(content, filePath) {
    const suggestions = [];
    const lines = content.split('\n');
    let priority = 'low';
...
function generateOptimizationHints(content, filePath) {
    const hints = [];
...
function generateDocumentationSuggestions(content, filePath) {
    const missing = [];
    const suggestions = [];
    const lines = content.split('\n');
