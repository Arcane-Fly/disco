...
import { validateContainerSession, } from '../middleware/sessionValidator.js';
const router = Router();
/**
 * GET /api/v1/files/:containerId
 * List directory contents
 */
router.get('/:containerId', validateContainerSession, async (req, res) => {
    try {
        const { path = '/' } = req.query;
        const session = req.session; // Validated by middleware
        // List files in the specified path
        const files = await listFiles(session.container, path);
        res.json({
            status: 'success',
            data: {
                path: path,
 ...
 * Get file content
 */
router.get('/:containerId/content', validateContainerSession, async (req, res) => {
    try {
        const { path } = req.query;
        const session = req.session; // Validated by middleware
...
async function listFiles(container, path) {
    try {
...
async function writeFile(container, path, content, encoding = 'utf-8') {
    try {
        const fs = container.fs;
...
async function deleteFile(container, path) {
    try {
        const fs = container.fs;
