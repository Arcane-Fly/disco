...
router.get('/', (req, res) => {
    ...
    const hasPlaceholders = Boolean((githubClientId && githubClientId.includes('your-github-client-id')) ||
...
router.get('/github', (req, res) => {
    ...
    // Check for placeholder values
    if (clientId.includes('your-github-client-id') ||
    ...
    const githubUrl = `https://github.com/login/oauth/authorize?` +
        ...
        `scope=user:email&` +
        `state=${state}`;
    console.log(`🔐 Initiating GitHub OAuth flow for redirect: ${safeRedirect}`);
    ...
    try {
        ...
        if (isChatGPTFlow) {
            ...
            const tempToken = jwt.sign({
                ...
                provider: 'github',
                temp: true,
            }, process.env.JWT_SECRET, { expiresIn: '10m' } // Short-lived for OAuth flow
            );
...
router.post('/login', async (req, res) => {
    try {
        ...
        // Create JWT token
        const token = jwt.sign({ userId, provider: 'api' }, process.env.JWT_SECRET, {
            expiresIn: '1h',
            issuer: 'mcp-server',
            audience: 'chatgpt',
        });
