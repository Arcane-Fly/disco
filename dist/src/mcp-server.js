    ...
    try {
        switch (name) {
            case 'create_container': {
                const userId = assertDefined(args?.userId, 'userId');
                const template = getStringOrDefault(args?.template, 'node');
            ...
            case 'execute_command': {
                const containerId = assertDefined(args?.containerId, 'containerId');
                const command = assertDefined(args?.command, 'command');
                const cwd = getStringOrDefault(args?.workingDirectory, '/');
            ...
            case 'create_file': {
                const containerId = assertDefined(args?.containerId, 'containerId');
                const path = assertDefined(args?.path, 'path');
                const content = assertDefined(args?.content, 'content');
                const encoding = getStringOrDefault(args?.encoding, 'utf-8');
                const session = await containerManager.getSession(containerId);
                if (!session) {
                    throw new Error(`Container ${containerId} not found`);
                }
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    switch (name) {
        case 'setup_project': {
            const projectType = getStringOrDefault(args?.type, 'node');
            const features = getStringOrDefault(args?.features, 'basic');
            return {
        ...
        case 'debug_container': {
            const containerId = assertDefined(args?.containerId, 'containerId');
            const issue = getStringOrDefault(args?.issue, 'general debugging');
            return {
