...
export class PocketFlow {
            ...
            for (const nodeName of sorted) {
                ...
                try {
                    ...
                    if (options?.stopOnError) {
                        throw new Error(`Node "${nodeName}" execution failed: ${error instanceof Error ? error.message : String(error)}`);
                    }
    ...
    topologicalSort() {
        ...
        if (result.length !== this.nodes.size) {
            throw new Error(`Workflow contains cycles. Expected ${this.nodes.size} nodes, but only ${result.length} are reachable.`);
        }
        return result;
    }
    /**
...
//# sourceMappingURL=pocketflow.js.map