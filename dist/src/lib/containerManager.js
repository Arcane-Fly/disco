...
class ContainerManager {
    sessions = new Map();
    pool = { ready: [], initializing: [] };
    cleanupInterval;
    maxInactiveMinutes;
    maxContainers;
    poolSize;
    circuitBreaker;
    constructor(maxInactiveMinutes = parseInt(process.env.CONTAINER_TIMEOUT_MINUTES || '15', 10), maxContainers = parseInt(process.env.MAX_CONTAINERS || '20', 10), poolSize = parseInt(process.env.POOL_SIZE || '3', 10)) {
     ...
     */
    async createSessionInternal(userId) {
        try {
            ...
            if (isServerEnvironment) {
                ...
                const session = {
                    ...
                    status: proxySession.status === 'running' ? 'ready' : proxySession.status,
                };
     ...
     */
    async initializeContainer(session) {
        try {
...
console.log('Current directory:', process.cwd());
    ...
    async cleanupInactive() {
        ...
        if (memoryPercentage > memoryThreshold && sessionsToCleanup.length === 0) {
            const sortedSessions = Array.from(this.sessions.entries()).sort(([, a], [, b]) => a.lastActive.getTime() - b.lastActive.getTime());
