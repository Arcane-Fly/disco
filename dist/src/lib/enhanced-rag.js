...
export class EnhancedRAGService {
    ...
    async indexCodebaseWithAST(container, options) {
    ...
    async performEnhancedSearch(container, options) {
    ...
    async parseFileWithAST(filePath, content) {
        const elements = [];
    ...
    async performASTSearch(options) {
        const results = [];
    ...
    async performEnhancedTextSearch(container, options) {
    ...
    async generateCodeSuggestions(query) {
...
router.get('/api/${this.extractEndpointName(query)}', async (req, res) => {
  try {
    ...
    // Helper methods
    calculateASTScore(element, query) {
        let score = 0;
        const queryLower = query.toLowerCase();
    ...
    calculateSemanticSimilarity(element, query) {
    ...
    calculateTextSemanticSimilarity(line, query) {
    ...
    calculateEnhancedScore(line, query, filePath) {
        let score = 0;
    ...
    hasSemanticMatch(line, query) {
        const lineLower = line.toLowerCase();
        const queryLower = query.toLowerCase();
        // Function patterns
        if (queryLower.includes('function') &&
            (lineLower.includes('function') || lineLower.includes('=>'))) {
            return true;
        }
        // Class patterns
    ...
    async findCodeFiles(container, rootPath = '.', excludePatterns = []) {
        const codeFiles = [];
        ...
        try {
            const findFiles = async (currentPath) => {
                try {
...
function generateArrowFunctionSignature(node) {
    const params = node.params
        .map((param) => {
        if (t.isIdentifier(param)) {
            return param.name;
...
function generateVariableSignature(node) {
    return `${node.id?.name || 'variable'} = ...`;
}
function extractClassDependencies(node) {
    const deps = [];
...
//# sourceMappingURL=enhanced-rag.js.map