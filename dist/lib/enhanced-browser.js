...
export class EnhancedBrowserAutomationManager {
    ...
    async createSession(containerId, config = {}) {
    ...
    async createPage(sessionId, url) {
    ...
    async takeEnhancedScreenshot(sessionId, pageId, options = {}) {
        const session = await this.getSession(sessionId);
        if (!session) {
        ...
        const { width = session.config.viewport.width, height = session.config.viewport.height, format = 'png', quality = 80, fullPage = false, element, } = options;
        try {
    ...
    async performVisualRegression(sessionId, pageId, testName, options = {}) {
    ...
    async simulateEnhancedClick(sessionId, pageId, options) {
        const session = await this.getSession(sessionId);
        if (!session) {
            throw new Error('Browser session not found');
        }
        ...
        const { x, y, selector, button = 'left', clickCount = 1, modifiers = [], waitForSelector = true, } = options;
        try {
            if (selector) {
    ...
    async performUIAutomation(sessionId, pageId, actions) {
        const session = await this.getSession(sessionId);
        if (!session) {
            throw new Error('Browser session not found');
        }
    ...
    async enableNetworkLogging(context) {
...
//# sourceMappingURL=enhanced-browser.js.map