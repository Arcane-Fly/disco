...
router.get('/audit-logs', authMiddleware, async (req, res) => {
    try {
        ...
        const limitedLogs = allLogs.slice(0, parseInt(limit, 10));
...
router.get('/incidents', authMiddleware, async (req, res) => {
    try {
        ...
        let incidents = securityComplianceManager.getSecurityIncidents(resolved !== undefined ? resolved === 'true' : undefined);
        if (severity) {
        ...
        await securityComplianceManager.logSecurityEvent(req, 'security_incidents_access', 'security_incidents', { filters: { resolved, severity }, resultCount: incidents.length });
        res.json({
            incidents,
            summary,
            timestamp: new Date().toISOString(),
 ...
 */
router.post('/incidents/:incidentId/resolve', authMiddleware, async (req, res) => {
    try {
        const user = req.user;
        if (!user?.userId) {
            return res.status(401).json({
        ...
        const success = await securityComplianceManager.resolveSecurityIncident(incidentId, resolution, user.userId);
        if (!success) {
            return res.status(404).json({
                error: 'Security incident not found',
            });
        }
        await securityComplianceManager.logSecurityEvent(req, 'security_incident_resolved', 'security_incident', { incidentId, resolution });
        res.json({
...
router.get('/compliance/reports', authMiddleware, async (req, res) => {
    try {
        ...
        await securityComplianceManager.logSecurityEvent(req, 'compliance_reports_access', 'compliance_reports', { reportCount: reports.length });
        res.json({
            reports,
            timestamp: new Date().toISOString(),
        });
 ...
 *         description: Unauthorized
 */
router.post('/compliance/generate-report', authMiddleware, async (req, res) => {
    try {
        const user = req.user;
        if (!user?.userId) {
            return res.status(401).json({
        ...
        await securityComplianceManager.logSecurityEvent(req, 'compliance_report_generated', 'compliance_report', { reportType: type, period: { start, end }, reportId: report.id });
        res.json(report);
    }
    catch (error) {
 ...
 *         description: Unauthorized
 */
router.post('/zero-trust/validate', authMiddleware, async (req, res) => {
    try {
        const user = req.user;
        if (!user?.userId) {
            return res.status(401).json({
...
router.get('/metrics', authMiddleware, async (req, res) => {
    try {
        ...
        await securityComplianceManager.logSecurityEvent(req, 'security_metrics_access', 'security_metrics', { metricsRequested: Object.keys(metrics) });
        res.json({
            ...metrics,
            timestamp: new Date().toISOString(),
        });
...
router.get('/status', authMiddleware, async (req, res) => {
    try {
        ...
        await securityComplianceManager.logSecurityEvent(req, 'security_status_check', 'security_status', { status, unresolvedIncidents: unresolvedIncidents.length });
