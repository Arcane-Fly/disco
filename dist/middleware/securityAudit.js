...
function determineAction(req) {
    ...
    if (path.includes('/auth')) {
        if (path.includes('/login') || path.includes('/token'))
            return 'authenticate';
 ...
 */
export const securityRateLimitMiddleware = (windowMs, max, message) => {
 ...
 */
export const securityInputValidationMiddleware = (req, res, next) => {
